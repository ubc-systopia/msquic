...
{
    uint8_t RawConfig[QUIC_DATAPATH_CONFIG_MIN_SIZE + 2 * sizeof(uint16_t)] = {0};
    QUIC_DATAPATH_CONFIG* Config = (QUIC_DATAPATH_CONFIG*)RawConfig;
    Config->ProcessorCount = 2;
    if (CxPlatProcActiveCount() < 2) {
        Config->ProcessorCount = CxPlatProcActiveCount();
    }
    ...
    QUIC_DATAPATH_CONFIG* GetConfig = (QUIC_DATAPATH_CONFIG*)GetRawConfig;
    ASSERT_EQ(
        QUIC_STATUS_SUCCESS,
        QuicLibraryGetGlobalParam(
            QUIC_PARAM_GLOBAL_DATAPATH_CONFIG,
    ...
    //
    // Passing a nullptr buffer should clear the proc list.
    //
    ASSERT_EQ(
        QUIC_STATUS_SUCCESS,
        QuicLibrarySetGlobalParam(
    ...
    //
    Config->ProcessorList[0] = (uint16_t)CxPlatProcActiveCount();
    ASSERT_EQ(
        QUIC_STATUS_INVALID_PARAMETER,
        QuicLibrarySetGlobalParam(
            QUIC_PARAM_GLOBAL_DATAPATH_CONFIG,
